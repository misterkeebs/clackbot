...
module.exports = async (iface, { channel, user, message, userData }) => {
  ...
  if (parts.length === 0 || hasClacks || !isMod) {
    ...
    if (hasClacks) {
      ...
      try {
        ...
        if (!theUser) {
          return await iface.reply(channel, user, 'você já está participando.');
        }
        return await iface.reply(channel, user, `você está participando com ${tickets} clacks. Você tem agora ${theUser.bonus} clacks.`);
      } catch (err) {
        if (err instanceof NoUserError) {
          return await iface.reply(channel, user, `infelizmente você não tem clacks prá participar desse sorteio.`);
        }
        if (err instanceof NotEnoughBonusError) {
          return await iface.reply(channel, user, `você não tem saldo suficiente prá participar com ${tickets} clacks. Use !clacks prá saber quantas vc tem.`);
        }

        // eslint-disable-next-line no-console
        console.error('Error adding user', user, 'to raffle', raffle, err);
        return await iface.reply(channel, user, `ocorreu um erro ao tentar te adicionar no sorteio. O erro foi: ${err}`);
      }
    }

    const { name, timeLeft } = raffle;
    return await iface.reply(channel, user, `estamos sorteando ${name}. O sorteio se encerra daqui ${timeLeft} minutos. Envie !sorteio <numero de clacks> para participar.`);
  }
  ...
  if (cmd.toLowerCase() === 'criar') {
    ...
    if (parts.length < 1 || isNaN(duration)) {
      return await iface.reply(channel, user, 'use !sorteio criar <duração em minutos> <nome>.');
    }
    const openRaffle = await Raffle.open();
    if (openRaffle) return await iface.reply(channel, user, `já existe sorteio aberto - ${openRaffle.name}`);
    const name = parts.join(' ');
    const raffle = await Raffle.create(name, duration);
    return await iface.reply(channel, user, `sorteio ${raffle.name} criado e aberto por ${duration} minutos! Mande !sorteio <clacks> para participar!`);
  }

  if (cmd.toLowerCase() === 'rodar') {
    const openRaffle = await Raffle.open();
    if (!openRaffle) return await iface.reply(channel, user, 'não existe sorteio aberto no momento.');
    ...
    await iface.reply(channel, user, 'iniciando abertura do sorteio.');
    return;
  }

  if (cmd.toLowerCase() === 'escolher') {
    if (!raffle) return await iface.reply(channel, user, 'não existe sorteio aberto no momento.');

    try {
      const winnerName = await raffle.run();
      return await iface.reply(channel, user, `o sorteado foi ${winnerName}! Parabéns!`);
    } catch (err) {
      if (err instanceof AlreadyRaffledError) {
        return await iface.reply(channel, user, `esse sorteio já foi rodado!`);
      }

      // eslint-disable-next-line no-console
      console.error('Error raffling', raffle, err);
      return await iface.reply(channel, user, `ocorreu um erro rodar o sorteio: ${err}`);
    }
  }
};
