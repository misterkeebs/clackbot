const yts = require("../../lib/vid-search");
const _ = require("lodash");

async function findVid(t) {
  const terms = t.toLowerCase();
  let r = await yts.search(`MrKeebs ${terms}`);
  const mrKeebsVid = r.videos.find(
    (v) => v.author && v.author.name === "MrKeebs",
  );
  ...
  const srTecladosVid = r.videos.find(
    (v) => v.author && v.author.name === "SrTeclados",
  );
...
const Vids = async (iface, { channel, user: userName, message }) => {
  ...
  if (!video) {
    return await iface.reply(
      channel,
      userName,
      `nenhum vídeo com ${terms} encontrado`,
    );
  }
