...
async function findVid(t) {
  ...
  if (mrKeebsVid.title.toLowerCase().includes(terms)) {
    return mrKeebsVid;
  }
  ...
  if (srTecladosVid.title.toLowerCase().includes(terms)) {
    return srTecladosVid;
  }
}

const Vids = async (iface, { channel, user: userName, message, rawMessage={} }) => {
