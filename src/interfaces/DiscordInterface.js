...
class DiscordInterface {
  constructor(bot, client) {
    ...
    this.client.on("message", async (msg) => {
      ...
      if (!user) {
        return msg.reply(
          `você precisa linkar a sua conta do Twitch. Prá isso, visite o chat do SrTeclados, visitando <https://www.twitch.tv/popout/SrTeclados/chat>, e digitando \`!eusou ${userName}\``,
        );
      }

      try {
        await this.bot.handleMessage(this, {
        ...
        this.reply(
          msg.channel.id,
          user.displayName,
          `houve um erro de processamento no seu comando: ${err}`,
        );
      }
    });
  }
